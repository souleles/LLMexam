...
5. **Greeklish Παραλλαγές για Ελληνικά Ονόματα Μεταβλητών/Πινάκων:**

   Όταν η άσκηση αναφέρει ονόματα πινάκων, μεταβλητών ή πεδίων με **Ελληνικές λέξεις**, πρόσθεσε πάντα και τη **Greeklish** τους απόδοση (η ίδια λέξη γραμμένη με λατινικούς χαρακτήρες). Οι φοιτητές συχνά γράφουν τα ονόματα μεταβλητών/πινάκων με λατινικά γράμματα αντί για ελληνικά.

   α) **Συνηθισμένα παραδείγματα Greeklish:**
      ```
      έδρα       → edra
      επιχείρηση → epixeirisi
      φοιτητής   → foititis
      τμήμα      → tmima
      ```

   β) **Χρήση εναλλακτικών για Ελληνικά + Greeklish + Αγγλικά:**
      ```
      (?i)(?:έδρα|edra)
      (?i)(?:FROM|JOIN)\\s+(?:επιχείρηση|epixeirisi|business|company)
      (?i)(?:φοιτητής|foititis|student)
      (?i)(?:τμήμα|tmima|department)
      ```

   γ) **Για ονόματα μεταβλητών (μερική αντιστοίχιση — χωρίς \\b):**
      ```
      (?i)(?:edra|έδρα) → πιάνει edra, edraId, edra_id, getEdra, κλπ.
      ```

   **Κανόνας:** Κάθε φορά που συναντάς Ελληνική λέξη που μπορεί να χρησιμοποιηθεί ως όνομα σε κώδικα (πίνακας, μεταβλητή, function, column), συμπερίλαβε και τη Greeklish παραλλαγή στο pattern με `(?:ελληνικά|greeklish)`.

6. **Patterns ανά Γλώσσα Προγραμματισμού:**
...
8. **Στρατηγική για Πολύπλοκες Απαιτήσεις:**
...
9. **Testing Regex Patterns - Checklist:**
   ...
   - ✅ Για Ελληνικά ονόματα (πίνακες, μεταβλητές, πεδία): περιλαμβάνει και τη Greeklish παραλλαγή;
...
SYSTEM_PROMPT_INDICATOR_SOLUTIONS = """Είσαι βοηθός που γράφει ενδεικτικές λύσεις σε κώδικα για patterns βαθμολόγησης ασκήσεων προγραμματισμού.

Για κάθε pattern δίνεται: ο αύξων αριθμός (order), η περιγραφή (description) και το regex pattern.

Γράψε για κάθε pattern ένα σύντομο **απόσπασμα κώδικα** (code snippet) που αποτελεί ενδεικτική λύση για αυτή την απαίτηση. Ο κώδικας πρέπει να είναι στη γλώσσα που αφορά η άσκηση (SQL, Python, JavaScript, κλπ.) και να είναι αρκετά σύντομος (μέχρι 5-8 γραμμές). Γράψε μόνο κώδικα — χωρίς εξηγήσεις, χωρίς σχόλια εκτός αν χρειάζονται για κατανόηση.

Επέστρεψε ΜΟΝΟ ένα έγκυρο JSON array. Κάθε αντικείμενο πρέπει να έχει:
- "order": ο αριθμός του pattern (ίδιος με αυτόν που δόθηκε)
- "indicatorSolution": η ενδεικτική λύση (string)

Χωρίς markdown fences, χωρίς εξηγήσεις. Ξεκίνα με `[` και τελείωνε με `]`."""

USER_PROMPT_INDICATOR_SOLUTIONS = """Ακολουθεί η άσκηση για context:
```
{extracted_text}
```
...
Γράψε μια σύντομη ενδεικτική λύση για κάθε pattern:"""
