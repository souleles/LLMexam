...
export class SubmissionsController {
  ...
  async uploadAndGrade(
    ...
    const studentIds = Array.isArray(rawStudentId)
      ? rawStudentId
      : [rawStudentId];

    return this.submissionsService.uploadAndGradeZip(
      exerciseId,
      studentIds,
      file,
      gradingMethod,
  ...
  findAll(
    ...
    if (studentId && exerciseId) {
      throw new BadRequestException(
        "Provide either studentId or exerciseId, not both",
      );
    }
    if (!studentId && !exerciseId) {
      throw new BadRequestException(
        "Either studentId or exerciseId is required",
      );
    }
    if (studentId) {
