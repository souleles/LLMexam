...
- **Dual grading methods** — Deterministic regex grading (via Python `re` module) is the primary method; LLM semantic grading is an optional secondary method for research comparison. Both results are stored side-by-side (see ADR-0002-amended).
...
- **Python microservice** — PDF extraction, LLM calls, regex grading, and semantic grading all live in the Python service; NestJS is the REST orchestrator and database owner (see ADR-0003).
...
- **Two chat flows** — Checkpoint generation and pattern generation are separate SSE endpoints and separate conversation histories (`type: CHECKPOINT | PATTERN`).
